...
export function buildSectionInsights(params: {
  ...
  insights.push({
    id: "overview",
    title: "전체 성과 요약",
    body: `이번 기간 총 광고비는 ${currency(summary.cost)}원이며, 클릭 ${formatNumber(summary.clicks)}회, 전환 ${formatNumber(summary.conversions)}건, 매출 ${currency(summary.revenue)}원, ROAS ${formatRoas(summary.roas)}입니다.`,
    tone: "neutral"
  });
  ...
  if (bestPlatform) {
    insights.push({
      id: "platform-best",
      title: "효율이 좋은 매체",
      body: `${bestPlatform.label} 매체가 ROAS ${formatRoas(bestPlatform.metrics.roas)}로 가장 높은 효율을 보였습니다.${biggestPlatform ? ` 비용 비중이 가장 큰 매체는 ${biggestPlatform.label}(${currency(biggestPlatform.metrics.cost)}원)입니다.` : ""}`,
      tone: "positive"
    });
  }
  ...
  if (weakCampaign) {
    insights.push({
      id: "campaign-warning",
      title: "캠페인 점검 포인트",
      body: `${weakCampaign.label} 캠페인은 광고비 ${currency(weakCampaign.metrics.cost)}원 대비 ROAS ${formatRoas(weakCampaign.metrics.roas)}를 기록했습니다. 비용 비중이 높다면 예산 낭비 구간인지 점검이 필요합니다.`,
      tone: "warning"
    });
  }
    ...
    insights.push(
      negativeKeyword
        ? {
            id: "keyword-negative",
            title: "제외 키워드 후보",
            body: `${negativeKeyword.label} 키워드는 클릭 ${formatNumber(negativeKeyword.metrics.clicks)}회, 전환 0건, 비용 ${currency(negativeKeyword.metrics.cost)}원으로 확인됩니다. 제외 키워드 후보로 검토할 수 있습니다.`,
            tone: "danger"
          }
        : {
            id: "keyword-negative",
  ...
  if (section === "creative") {
    ...
    if (bestCreative) {
      insights.push({
        id: "creative-best",
        title: "소재 인사이트",
        body: `${bestCreative.label} 소재는 CTR ${formatPercent(bestCreative.metrics.ctr, 1)}로 가장 높은 반응을 보였습니다. 후킹 요소와 메시지 구조를 확장 테스트하기 좋은 후보입니다.`,
        tone: "positive"
      });
    }
  }
  ...
  if (section === "landing") {
    ...
    if (weakLanding) {
      insights.push({
        id: "landing-warning",
        title: "랜딩페이지 점검",
        body: `${weakLanding.label} 랜딩페이지는 비용 ${currency(weakLanding.metrics.cost)}원 대비 CVR ${formatPercent(weakLanding.metrics.cvr, 1)}입니다. 유입 후 전환 흐름 점검이 필요합니다.`,
        tone: "warning"
      });
    }
  }
